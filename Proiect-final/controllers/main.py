...
import sys
from controllers.foodapi import ApiClient
from views.nutrition_widget import NutritionInfoWidget
from views.meal_history_widget import MealHistoryWidget

from model.models import User, Meal, get_db  
...
class NutritionApp(QMainWindow):
    def __init__(self, user=None):
        ...
        meals_button.clicked.connect(self.open_meal_history)  
    ...
    def load_food_list(self):
        try:
            # Obține calea absolută a fișierului curent (controllers/nutritionapp.py)
            current_dir = os.path.dirname(os.path.abspath(__file__))
            # Obține calea către folderul principal al proiectului (urcă un nivel)
            project_root = os.path.abspath(os.path.join(current_dir, ".."))
            # Construiește calea completă către data/foods.txt
            food_file_path = os.path.join(project_root, "data", "foods.txt")

            with open(food_file_path, "r", encoding="utf-8") as f:
                foods = [line.strip() for line in f if line.strip()]
            return foods
        except Exception as e:
            QMessageBox.warning(self, "Warning", f"Could not load food list from data/foods.txt.\n{e}")
            return []
    ...
    def open_meal_history(self):
        if not self.user_id:
            QMessageBox.warning(self, "Warning", "No user logged in.")
            return
        self.meal_history_window = MealHistoryWidget(self.user_id)
        self.meal_history_window.show()
